...
class MyCallbacks : public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) override {
        ...
        if (value.length() > 0) {
            ...
            if (value == "MSG from client touch screen") {
                pCharacteristic->setValue("Prosthesis' Server recived MSG from client touch screen");
...
void BLE_server_start(void* param = NULL) {
  delay(1500);
  ...
  pCharacteristic->setValue("Connected to the Prosthesis' server");
