...
void return_to_main(lv_event_t *e) {    
    ...
    if(lv_event_get_code(e) == LV_EVENT_CLICKED) {
      ...
      is_tech = false;
      is_user = false;
      // xTaskCreate(return_BLE,"notify", STACK_SIZE, nullptr, 2, nullptr);
      ...
      if (xSemaphoreTake(xMutex, portMAX_DELAY) == pdTRUE){
        if (!confirmationReceived.load(std::memory_order_acquire)) {
          Serial.println("NO");
      ...
      confirmationReceived.store(false, std::memory_order_release); 
      lv_scr_load(initial_user_screen); // Return to the main screen
